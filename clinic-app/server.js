...
// Mount API Routes
const clinicAuthRoutes = require('./src/routes/authRoutes');
...
app.use((req, res) => {
  ...
  if (req.path.startsWith('/v1/') || req.path.startsWith('/webhooks/')) {
    return res.status(404).json({
      success: false,
      error: { code: "NOT_FOUND", message: "هذا الرابط غير موجود في الخادم" }
    });
  }
  // Otherwise serve the index.html for SPA-like behavior
  res.sendFile(path.join(__dirname, 'public', 'index.html'));
});
...
const server = app.listen(PORT, () => {
  console.log(`\n======================================================`);
  console.log(`🏥 Smart Clinic OS — Clinic App is running on port ${PORT}`);
