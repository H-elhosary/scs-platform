...
async function renderCalendar() {
  ...
  container.innerHTML = '<div style="padding:40px; text-align:center; color:#94a3b8;"><i class="fa-solid fa-spinner fa-spin fa-2x"></i><p style="margin-top:10px;">جاري تحميل المواعيد...</p></div>';

  try {
    const res = await fetch(`${API_BASE}/v1/appointments?t=${Date.now()}`).then(r => r.json());
    if (res.success) {
...
function renderMonthView(container, appointments, dayNames, monthNames) {
  ...
  dayNames.forEach(d => { html += `<div class="cal-month-day-name">${d}</div>`; });

  // Render prefix days from prev month
  for (let i = firstDayIndex; i > 0; i--) {
    const dNum = prevMonthTotalDays - i + 1;
    html += `<div class="cal-month-cell other-month"><span class="cal-month-cell-num">${dNum}</span></div>`;
  }
  ...
  for (let d = 1; d <= totalDays; d++) {
    ...
    html += `<div class="cal-month-cell ${isToday ? 'today' : ''}" onclick="showNewAppointmentModal('${curDateStr}')">
      <span class="cal-month-cell-num">${d}</span>
      <div class="cal-month-cell-events">
        ${dayAppts.slice(0, 3).map(a => `<div class="cal-month-event cal-event status-${a.status}" onclick="event.stopPropagation(); showAppointmentDetails('${a.id}')">${a.time} ${a.patient_name}</div>`).join('')}
  ...
  const remainingCells = 42 - totalCells; // 6 rows standard
  for (let i = 1; i <= remainingCells; i++) {
    html += `<div class="cal-month-cell other-month"><span class="cal-month-cell-num">${i}</span></div>`;
  }
...
function showNewAppointmentModal(date, time) {
  ...
  if (date) document.getElementById('apt-date').value = date;
  else document.getElementById('apt-date').value = formatDate(new Date());
...
async function updateAvailableSlots() {
  ...
  try {
    const whRes = await fetch(`${API_BASE}/v1/settings/working-hours?doctor_id=${doctorId}`).then(r => r.json());
    ...
    const aptsRes = await fetch(`${API_BASE}/v1/appointments?doctor_id=${doctorId}&date=${dateInput}`).then(r => r.json());
  ...
  try {
    ...
    if (res.success) {
      ...
      if (confirm(confirmMsg)) {
        const updateRes = await fetch(`${API_BASE}/v1/appointments/${appointmentId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            date: newDate,
...
async function showAppointmentDetails(appointmentId) {
  try {
    ...
    if (res.success) {
      ...
      document.getElementById('detail-patient-name').innerText = apt.patient_name || 'غير معروف';
      
      // Find doctor name
      const doctor = allDoctors.find(d => d.id === apt.doctor_id);
      document.getElementById('detail-doctor-name').innerText = doctor ? doctor.full_name : 'د. محمد نور';
      
      document.getElementById('detail-service-name').innerText = `${apt.service_name} — ${apt.amount} ج.م`;
      document.getElementById('detail-location').innerText = apt.location || 'العيادة الرئيسية';
      
      // Date and Time formatted
      const displayTime = formatTimeArabic(apt.time);
      ...
      if (apt.payment_method === 'online') payStr = 'دفع إلكتروني';
      else if (apt.payment_method === 'insurance') payStr = 'تأمين طبي';
      document.getElementById('detail-payment-method').innerText = payStr;
      
      // Notes
      document.getElementById('detail-notes').innerText = apt.notes || 'لا توجد ملاحظات إضافية.';
...
async function cancelAppointmentFromCalendar() {
  ...
  if (confirm('هل أنت متأكد من رغبتك في إلغاء وحذف هذا الحجز نهائياً؟')) {
    try {
      const res = await fetch(`${API_BASE}/v1/appointments/${appointmentId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          status: 'cancelled'
