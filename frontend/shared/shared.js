...
async function loadGlobalData() {
  try {
    const [pRes, sRes, whRes, dRes] = await Promise.all([
      fetch(`${API_BASE}/v1/patients`).then(r => r.json()),
      fetch(`${API_BASE}/v1/settings/services`).then(r => r.json()),
      fetch(`${API_BASE}/v1/settings/working-hours`).then(r => r.json()),
      fetch(`${API_BASE}/v1/doctors`).then(r => r.json())
    ]);
